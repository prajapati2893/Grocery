...
include('admin/inc/inc.php');
 ...
 $srchprod = !empty($_POST['search']) ? $_POST['search'] : "";
 $srchitm = str_replace("=","",str_replace("'","", $srchprod));
  if($srchitm=="")
  {
   $featured=mysqli_query($conn, "SELECT * FROM tbl_products where prod_status='1' order by pid desc");
  }
  else
  {

     $featured=mysqli_query($conn, "SELECT * FROM tbl_products where (pname Like '%".$srchitm."%' or details like '%".$srchitm."') and  prod_status='1' order by pid desc");
  }
                ...
                <div class="d-flex">
                  ...
                  <?php $pric= $row['pprice']- ($row['pprice']*$row['discount_percent'])/100; } else { $pric= $row['pprice']; } ?>
                    <p class="price">
                      ...
                      <span class="price-sale" >Rs.<?php echo number_format($pric,2); ?></span>/per <?php echo $row['unit_type'];?></p>
                  </div>
