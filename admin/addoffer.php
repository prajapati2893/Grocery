...
{
    $imgpath='';
    if(!empty($_FILES['ofrimg']['name'])){
 //if(isset($_FILES['profile_photo']['name'])!=''){
      $errors= array();
      $banerimg= $file_name = $_FILES['ofrimg']['name'];
       $file_size =$_FILES['ofrimg']['size'];
      $file_tmp =$_FILES['ofrimg']['tmp_name'];
      $file_type=$_FILES['ofrimg']['type'];
   ...
   {
      $imgs ='';
      if($imgpath!=''){ $imgs = ", offer_image='".$imgpath."'"; }
       
        $sql=mysqli_query($conn," UPDATE `tbl_offerbaner` SET `offer_title` = '".$title."',offer_text='".$offer_text."' ".$imgs." WHERE `offer_id` = '".$offer_id."'");
   ...
   else
   {
    ...
    $sql=mysqli_query($conn,"INSERT INTO `tbl_offerbaner` (`offer_id`, `offer_title`,`offer_text`, `offer_image`, `adddate`) VALUES (NULL, '".$title."', '".$offer_text."', '".$imgpath."', CURRENT_TIMESTAMP)");
    $row=mysqli_fetch_array($sql);
   }
header('location:offer.php');
}

$gtoffer_id = !empty($_REQUEST['ofrid'])?$_REQUEST['ofrid']:"";
if($gtoffer_id!=''){
$sql=mysqli_query($conn,"SELECT * FROM tbl_offerbaner where offer_id='".$gtoffer_id."'");
$row=mysqli_fetch_array($sql);
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                     <form method="post" enctype="multipart/form-data" action="" >
                                        <input type="hidden" name="offer_id" value="<?php echo $gtoffer_id;?>">
                                             <div class="form-row">
                                                <div class="col-md-6">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control py-4" id="inputLastName" type="file"  name="ofrimg" placeholder="Select image" <?php if($gtoffer_id=='') { ?>required="required" <?php } ?> >
                                                    </div>
                                                </div>
                                                 <div class="col-md-6">
                                                    <div class="form-group">
                                                        <label class="small mb-1" for="inputLastName">Offer Details Text</label>
                                                        <textarea  class="form-control py-4" name="offer_text" <?php if($gtoffer_id=='') { ?>required="required" <?php } ?>><?php echo $offer_text;?></textarea> 
                                                    </div>
                                                </div>
                                            </div>
