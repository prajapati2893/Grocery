...
{
    $imgpath='';
    if(!empty($_FILES['banrimg']['name'])){
 //if(isset($_FILES['profile_photo']['name'])!=''){
      $errors= array();
      $banerimg= $file_name = $_FILES['banrimg']['name'];
       $file_size =$_FILES['banrimg']['size'];
      $file_tmp =$_FILES['banrimg']['tmp_name'];
      $file_type=$_FILES['banrimg']['type'];
   ...
   {
      $imgs ='';
      if($imgpath!=''){ $imgs = ", bimgname='".$imgpath."'"; }
       
        $sql=mysqli_query($conn," UPDATE `tbl_topbanner` SET `btitle` = '".$title."' ".$imgs." WHERE `bid` = '".$bid."'");
   ...
   else
   {
    ...
    $sql=mysqli_query($conn,"INSERT INTO `tbl_topbanner` (`bid`, `btitle`, `bimgname`, `adddate`) VALUES (NULL, '".$title."', '".$imgpath."', CURRENT_TIMESTAMP)");
    $row=mysqli_fetch_array($sql);
   }
header('location:topbanner.php');
}

$gtbid = !empty($_REQUEST['bid'])?$_REQUEST['bid']:"";
if($gtbid!=''){
$sql=mysqli_query($conn,"SELECT * FROM tbl_topbanner where bid='".$gtbid."'");
$row=mysqli_fetch_array($sql);
$title = $row['btitle'];
}
else
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                     <form method="post" enctype="multipart/form-data" action="" >
                                        <input type="hidden" name="bid" value="<?php echo $gtbid;?>">
                                             <div class="form-row">
                                                <div class="col-md-6">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control py-4" id="inputLastName" type="file"  name="banrimg" placeholder="Select image" <?php if($gtbid=='') { ?>required="required" <?php } ?> >
                                                    </div>
                                                </div>
                                            </div>
